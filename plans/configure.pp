plan pe_xl::configure (
  ...
  Optional[Boolean]   $run_from_primary = true,
  ...
  if $run_from_primary {
    ...
    # leave default transport for primary_master_host to local
    $primary_master_host_local = $primary_master_host
  }
  ...
  if $control_repo {
    run_task('pe_xl::code_manager', $primary_master_host_local,
      action => 'deploy',
    ...
    run_task('pe_xl::download', $primary_master_host_local,
    ...
    run_command("/opt/puppetlabs/bin/puppet module install /tmp/${nm_module_tarball}", $primary_master_host_local)
    run_command("/opt/puppetlabs/bin/puppet module install /tmp/${pexl_module_tarball}", $primary_master_host_local)
    run_command('chown -R pe-puppet:pe-puppet /etc/puppetlabs/code', $primary_master_host_local)
  }

  run_task('pe_xl::configure_node_groups', $primary_master_host_local,
  ...
  run_task('pe_xl::pe_puppet_run', $primary_master_host_local)
  ...
  run_task('pe_xl::provision_replica', $primary_master_host_local,
    primary_master_host    => $primary_master_host,
  ...
  run_task(pe_xl::configure_replica_db_node_group, $primary_master_host_local,
