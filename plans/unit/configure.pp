...
plan pe_xl::unit::configure (
  ...
  run_task('pe_xl::configure_node_groups', $master_target,
    master_host                    => $master_target[0].host,
    master_replica_host            => $master_replica_target[0].host,
    puppetdb_database_host         => $puppetdb_database_target[0].host,
    puppetdb_database_replica_host => $puppetdb_database_replica_target[0].host,
  ...
  if $arch['high-availability'] {
    ...
    run_task('pe_xl::provision_replica', $master_target,
      master_replica => $master_replica_target[0].host,
