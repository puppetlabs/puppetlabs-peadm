...
describe 'peadm::convert' do
  ...
  # This before(:each) and its tests are scoped to their own context (rather
  # than living at the top of the outer describe block) so that the
  # pe_compiler_legacy-specific task expectations below don't leak into the
  # unrelated availability-group contexts further down, which set up their
  # own distinct peadm::cert_data/node_group_unpin/etc. expectations.
  context 'legacy compiler classification order' do
    before(:each) do
      allow_out_message
      allow_any_command
      allow_apply
